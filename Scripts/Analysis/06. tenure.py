...
if 'department' in df.columns: #remove temp department
...
diag_table = df.groupby(['tenure', 'status_label'], observed=False).size().unstack(fill_value=0)
...
ax = sns.countplot(data=df, x='tenure', hue='status_label', hue_order=status_order, palette=colors, edgecolor='white')
for p in ax.patches:
    if p.get_height() > 0:
...
plt.title('Attrition Distribution by Tenure (D1)', fontsize=15, pad=20)
plt.xlabel('Tenure', fontsize=12)
plt.ylabel('Number of Employees', fontsize=12)
sns.despine()
plt.tight_layout()
plt.savefig(os.path.join(SAVE_PATH, "06_D1_Tenure.png"), dpi=300)
diag_table.to_csv(os.path.join(TABLE_PATH, "Table_4_D1_Tenure.csv"))